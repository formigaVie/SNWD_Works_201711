...
if __name__ == '__main__':
    ...
    s_links = adapt_soup.findAll("a", attrs={"href": re.compile(r"/wiki/Game_of_Thrones_\(season_\d+\)")})
    s_links = set([link["href"] for link in s_links])

    for s_link in s_links:
        full_season_url = urlwp + s_link
        season_page = urllib2.urlopen(full_season_url).read()
        sl_soup = BS.BeautifulSoup(season_page)
        ...
        if table:
            ...
            for idx, episode_view in enumerate(episode_views):
                if episode_view:
                    ...
                    print episode_viewers, viewers, s_link, idx
                    viewers += float(re.sub("\[[0-9]\]", "", almost))
